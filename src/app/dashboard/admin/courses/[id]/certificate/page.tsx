...
export default async function CourseCertificatePage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-black text-brand-white">قالب شهادة: {course.title}</h1>
        <p className="text-brand-silver-dim mt-1 text-sm">لو صممت قالب خاص هون، بيستخدمه بدل القالب الافتراضي لهذه الدورة بس.</p>
      </div>
      <CertificateTemplateBuilder
        fetchUrl={`/api/admin/courses/${params.id}/certificate-template`}
        saveUrl={`/api/admin/courses/${params.id}/certificate-template`}
        title={course.title}
      />
    </div>
  );
